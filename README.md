...
2. Build mlflow server image and launch. Run `docker build -t localhost:5000/mlflow_server ./mlflow_server_docker/` to build. Run `docker run -d --name mlflow_server --restart always --net=host -v /var/experiment_data/artifact:/artifact localhost:5000/mlflow_server` to launch the mlflow server.
